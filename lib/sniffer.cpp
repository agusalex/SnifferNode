...
void upload(IPAddress ip, int port,
    ...
    /*    for (int i = 0; i < data_index; i += aggregation) {

         char buffer[1020];
         int offset = sprintf(buffer, "[");


      for (int j = i; j < i + aggregation && i + aggregation <= data_index; j++) {
             int rssi = RSSI_DATA[j];
             unsigned int seq = SEQ_DATA[j];
             offset += sprintf(buffer + offset, R"({"s":%u,"r":%ITER})", seq, rssi);
             if (j + 1 < i + aggregation) {
                 offset += sprintf(buffer + offset, ",");
             }
         }
         sprintf(buffer + offset, "]");
